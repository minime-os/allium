...
lazy_static! {
    pub static ref ALLIUM_BASE_DIR: PathBuf = PathBuf::from(
        &env::var("ALLIUM_BASE_DIR").map_or_else(|_| ALLIUM_SD_ROOT.join(".ui"), PathBuf::from)
    );
    ...
    pub static ref ALLIUM_THEMES_DIR: PathBuf = ALLIUM_BASE_DIR.join("themes");
    ...
    pub static ref ALLIUM_SCREENSHOTS_DIR: PathBuf = ALLIUM_SD_ROOT.join("saves/screenshots");
    ...
    pub static ref ALLIUM_DATABASE: PathBuf = env::var("ALLIUM_DATABASE")
        .map(PathBuf::from)
        .unwrap_or_else(|_| ALLIUM_BASE_DIR.join("state/allium.db"));
