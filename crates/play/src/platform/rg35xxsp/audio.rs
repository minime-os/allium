// RG35xxSP host audio output using dynamically loaded ALSA (libasound.so.2).
        ...
        let thread = thread::Builder::new()
            ...
            .spawn(move || {
                if let Err(err) = run_alsa_thread(sample_rate, consumer, thread_running) {
                    warn!("RG35xxSP dynamic ALSA audio thread stopped: {:#}", err);
                }
            })
            .context("Failed to spawn RG35xxSP dynamic ALSA audio thread")?;
